...
%% Import Data
current_path = pwd;
pathname_taux = fullfile(current_path, 'piControlData/CanESM2/CanESM2_taux.cdf');     % CDF file from Lamont website

taux = ncread(pathname_taux, 'tauuo');
time = ncread(pathname_taux, 'T');

pathname_coord = fullfile(current_path, 'piControlData/CanESM2/tauuo_Omon_CCSM4_piControl_r1i1p1_025001-050012.nc' );                      % CDF file from Lamont website does not contain lat and lon variables. 'pathname_coord' file downloaded from ESGF website to obtain these variables.
...
title('CanESM2 Surface Windstress - Original Grid', 'fontsize', 12)
colorbar
set(gca, 'fontsize', 12)
...
save('CanESM2_windstress.mat', 'taux', 'time', '-v7.3');
